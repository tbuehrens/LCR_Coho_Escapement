...
parameters{
  ...
  //pop-specific GRTS zero inflation
  real<lower=0,upper=1> p_zero_mu;
  real<lower=0> sigma_zero;
  vector[P] eps_p_zero;
  //P_Index_both
...
transformed parameters{
  ...
  //GRTS redd zero inflation
  vector<lower=0,upper=1>[P] p_zero = inv_logit(logit(p_zero_mu) + eps_p_zero * sigma_zero);
  //Total Abundance
  matrix[T,P] Adults;
  //UM Abundance
  matrix[T,P] UM_ad;
...
model{
  ...
  //pop-specific GRTS zero inflation
  p_zero_mu ~ beta(0.5,0.5);
  sigma_zero ~ std_normal();
  eps_p_zero  ~ std_normal();
  //P_Index_both
