...
#datasets
#accdb.name<-"TWS_Analysis_Only_v9.1.accdb"
# traphauldata.name<-"TH_dat_04022020_w_nflewisabvdam.csv"
# frame_file.name<-"Coho_frames_07.19.2020.csv"
# RpF_file.name<-"Coho_RpF_04.08.2020.csv"
# IMWdata.name<-"IMW_redds_02.14.2020.csv"
# IMWdata_v2.name<-"IMW_redds_06_25_2021_V2.csv"
accdb.name<-"TWS_Analysis_Only_v9.2.accdb"
traphauldata.name<-"TH_dat_06_29_21_w_nflewisabvdam.csv"
frame_file.name<-"Coho_frames_07.19.2020.csv"
RpF_file.name<-"Coho_RpF_04.14.2021.csv"
IMWdata.name<-"IMW_redds_01.27.2021.csv"
IMWdata_v2.name<-"IMW_redds_06_25_2021_V2.csv"


# set data summarization params
years=c(2010:2020)
GRTSdat.date<-"2021-06-29"
...
stan_model.file<-"CohoEscapement_Model_6_25_2021.stan"
...
2. it then queries TWS and generates identically formated csv files from TWS for years after 2011 and puts them in the same folder.
...
if(UpdateGRTSdata==TRUE){
  if(2010 %in% years){
    GRTSdataprep2010(data.dir=data.dir,frame_file.name=frame_file.name)
  }
  if(2011 %in% years){
    GRTSdataprep2011(data.dir=data.dir,frame_file.name=frame_file.name)
  }
...
IMW_Y2<-read_csv(file.path(csv.data.dir,IMWdata_v2.name))%>%
  left_join(GRTSpops%>%
    ...
    rename(yr_Y2 = Year, pop_Y2 = Order)%>%
...
g<-melt(jagsdat$g)%>%
  as_tibble()%>%
  rename(pop_Y = X1, Reach = X2, yr_Y = X3, g = value)

Y<-melt(jagsdat$y)%>%
  as_tibble()%>%
  rename(pop_Y = X1, Reach = X2, yr_Y = X3, Y = value)%>%
  ...
  rename(GRTS_Population=pop_Y,pop_Y=Order)%>%
...
catch_a_M_mu_dat<-jagsdat$log_catch_a_M_mu%>%
  ...
  left_join(years%>%
              as_tibble()%>%
              rownames_to_column()%>%
              rename(crc_years=rowname,crc_years_real=value)%>%
...
df<-data.frame(melt(res$pF))%>%
  rename(pF=value,Year=Var.3,GRTS_Population = Var.2)
