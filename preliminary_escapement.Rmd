...
#=====================
f_year<-2025 #this is the year we want to predict prelim escapement for
results_folder<-"Results_2025-06-19" #trib estimate results to use.
wild_results<-"2010_2024_Coho_TRT_UM_ad.csv"
total_results<-"2010_2024_Coho_TRT_Adults.csv"
#===========================
...
if(!file.exists("results/prelim/m1.RDS")){
  ...
  m1<-stan_gamm4(log(LCR.wild) ~ 
               s(Year,bs="ps") +
               log(Bonneville.total) + 
               log(Willamette.total) +
               log(Clackamas.w) +
               log(Cowlitz.barrier.total)+
               log(Washougal.hatchery.late.h),
...
if(!file.exists("results/prelim/m3.RDS")){
  if(!file.exists("models/stan_prelim_model.RDS")){  
    url  <- "https://raw.githubusercontent.com/tbuehrens/WDFW_SOS_Analysis/2022/models/model_mv_v3.stan"
    dest <- here::here("models/stan_prelim_model.stan")
    download.file(url, destfile = dest, method = "wininet", mode = "wb")
    model<-stan_model("models/stan_prelim_model.stan")
