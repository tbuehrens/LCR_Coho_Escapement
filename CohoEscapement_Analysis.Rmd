...
p <- ggplot(Y_PPC, aes(x = Y, y = Y, color=subpopulation)) +
  facet_grid(rows=vars(subpopulation), scales = "free") +
...
p <- ggplot(Y_PPC, aes(x = Y+1, y = Y+1)) +
  geom_point(aes(y=mean))+
  labs(x = substitute(italic(r)), y = "95% PPI") +
  geom_line(color="black") +
  scale_x_log10()+
...
p <- ggplot(Y_PPC, aes(x = Y+1, y = Y+1)) +
  geom_point(aes(x = jitter(Y+1),y=mean))+
  labs(x = substitute(italic(r)), y = "Posterior Mean GRTS Redds") +
  geom_line(color="black") +
  scale_x_log10()+
  scale_y_log10()+
  theme(text = element_text(size = 12))
