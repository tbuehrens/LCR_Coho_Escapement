...
p<-ggplot(H_PPC, aes(x = H, y = H, color=subpopulation,group=year)) +
  geom_errorbar(aes(ymin = `2.5%`, ymax = `97.5%`), width = 0) +
  labs(x = "Observed H", y = "95% PPI") +
...
p <- ggplot(Y_PPC, aes(x = Y, y = Y, color=subpopulation, group=year)) +
  facet_grid(cols=vars(subpopulation), rows=vars(year), scales = "free") +
  geom_errorbar(aes(ymin = `2.5%`, ymax = `97.5%`), width = 0) +
  labs(x = "Observed Y", y = "95% PPI") +
  geom_line(color="black") +
...
p <- ggplot(Y_PPC, aes(x = Y+1, y = Y+1, color=fit)) +
...
p<-ggplot(TH_a_UM_PPC, aes(x = TH_a_UM, y = TH_a_UM, color=subpopulation,group=year)) +
  geom_errorbar(aes(ymin = `2.5%`, ymax = `97.5%`), width = 0) +
  labs(x = "Observed TH_a_UM", y = "95% PPI") +
  geom_line(color="black") +
...
p<-ggplot(TH_a_M_PPC, aes(x = TH_a_M, y = TH_a_M, color=subpopulation,group=year)) +
  geom_errorbar(aes(ymin = `2.5%`, ymax = `97.5%`), width = 0) +
  labs(x = "Observed TH_a_M", y = "95% PPI") +
  geom_line(color="black") +
...
p<-ggplot(catch_a_M_mu_PPC, aes(x = catch_a_M_mu, y = catch_a_M_mu, color=subpopulation,group=year)) +
  geom_errorbar(aes(ymin = `2.5%`, ymax = `97.5%`), width = 0) +
  labs(x = "Observed catch_a_M_mu", y = "95% PPI") +
  geom_line(color="black") +
...
trends<-res$eps_lambda%>%
  reshape2::melt()%>%
  setNames(c("iteration","GRTS_Population","Year","eps_lambda"))%>%
  ...
  bind_rows(expand_grid(Year=years[1],GRTS_Population=c(GRTSpops$GRTS_Population, "WA LCR ESU"),mean=0,sd=0,`2.5%`=0,`25%`=0,`50%`=0,`75%`=0,`97.5%`=0))%>%
