...
fc6 <- "Figure S6. Regression of observed vs. 95% posterior predictive interval for GRTS redd counts $r$. Both x and y axes are log10 transformed and a constant of 1 was added to observations and predictions to aid visualization. Coloring depicts observations within and outside of the 95% PPI. A jitter was applied to x values. Lack of fit is indicated by PPI that do not overlap the black line."
...
fc10 <- "Figure S10. Regression of observed vs. 95% posterior predictive interval for GRTS redd counts $r$. Panels are years (rows) and subpopulations (columns) and data are individual reach-level redd counts. Lack of fit is indicated by PPI that do not overlap the black line."
```
...
knitr::include_graphics(here::here(file.path("results",results_name,"Y_PPC_v3.png")))
```
